...
} from "../../qq-classic-farm/ios/qq-farm-store.mjs";
import {
  detectBestQqFarmTemplateMatch,
  detectQqFarmTemplateMatches,
  listQqFarmTemplateNames,
} from "../../qq-classic-farm/ios/qq-farm-template-fallback.mjs";
import { detectQqFarmVisualSnapshot } from "../../qq-classic-farm/ios/qq-farm-vision.mjs";
import {
  classifyQqFarmSceneWithSidecars,
  locateQqFarmUiPointWithSidecars,
} from "../../qq-classic-farm/ios/qq-farm-vision-sidecars.mjs";
...
const QQ_FARM_SPEC_PATH = path.join(REPO_ROOT, "qq-classic-farm", "src", "qq-farm-spec.json");
...
function loadQqFarmSpec() {
  try {
    ...
    return {
      ...
      actions: {
        warehouseEntryTexts: ["仓库"],
        warehouseSellTexts: ["一键出售", "全部出售", "出售"],
        homeOneKeyActions: [
        ...
        friendOneKeyActions: [
          { id: "harvest", note: "一键摘取", texts: ["一键摘取", "一键采摘", "一键收获", "一键偷取", "一键偷菜", "摘取", "采摘", "收获", "偷取", "偷菜"] },
          { id: "water", note: "一键浇水", texts: ["一键浇水", "浇水"] },
          { id: "weed", note: "一键除草", texts: ["一键除草", "除草"] },
          { id: "pest", note: "一键除虫", texts: ["一键除虫", "除虫"] },
        ],
        friendEntryTexts: ["好友"],
        friendVisitTexts: ["拜访"],
        friendHelpTexts: ["求助", "帮忙", "去帮忙", "需要帮忙", "好友求助"],
        returnHomeTexts: ["回家"],
        storeEntryTexts: ["商店"],
        mallEntryTexts: ["商城"],
        mallFertilizerTexts: ["10小时有机化肥", "10小时", "有机化肥", "化肥"],
        purchaseConfirmTexts: ["购买", "立即购买", "确定", "确认"],
...
const QQ_FARM_WAREHOUSE_ENTRY_TEXTS = uniqueTexts(QQ_FARM_SHARED_SPEC.actions?.warehouseEntryTexts || ["仓库"]);
const QQ_FARM_WAREHOUSE_SELL_TEXTS = uniqueTexts(QQ_FARM_SHARED_SPEC.actions?.warehouseSellTexts || ["一键出售", "全部出售", "出售"]);
...
const QQ_FARM_FRIEND_HELP_TEXTS = uniqueTexts(QQ_FARM_SHARED_SPEC.actions?.friendHelpTexts || []);
const QQ_FARM_FRIEND_LIST_TAB_TEXTS = uniqueTexts(
  Object.values(QQ_FARM_SHARED_SPEC.actions?.friendTabTexts || {}).flat(),
);
const QQ_FARM_RETURN_HOME_TEXTS = uniqueTexts(QQ_FARM_SHARED_SPEC.actions?.returnHomeTexts || ["回家"]);
const QQ_FARM_STORE_ENTRY_TEXTS = uniqueTexts(QQ_FARM_SHARED_SPEC.actions?.storeEntryTexts || ["商店"]);
const QQ_FARM_MALL_ENTRY_TEXTS = uniqueTexts(QQ_FARM_SHARED_SPEC.actions?.mallEntryTexts || ["商城"]);
const QQ_FARM_MALL_FERTILIZER_TEXTS = uniqueTexts(
  QQ_FARM_SHARED_SPEC.actions?.mallFertilizerTexts || ["10小时有机化肥", "10小时", "有机化肥", "化肥"],
);
const QQ_FARM_PURCHASE_CONFIRM_TEXTS = uniqueTexts(
  QQ_FARM_SHARED_SPEC.actions?.purchaseConfirmTexts || ["购买", "立即购买", "确定", "确认"],
);
...
const CLEAR_WECHAT_AFTER_QQ_FARM = resolveBooleanEnv(
  [
    process.env.WEIXIN_IOS_CLEAR_WECHAT_AFTER_QQ_FARM,
...
const DEFAULT_QQ_FARM_FRIEND_CAROUSEL_LEFT_SWIPE_FROM_COORD = JSON.stringify({ xRatio: 0.74, yRatio: 0.901 });
const DEFAULT_QQ_FARM_FRIEND_CAROUSEL_LEFT_SWIPE_TO_COORD = JSON.stringify({ xRatio: 0.28, yRatio: 0.901 });
const DEFAULT_QQ_FARM_FRIEND_CAROUSEL_RIGHT_SWIPE_FROM_COORD = JSON.stringify({ xRatio: 0.28, yRatio: 0.901 });
const DEFAULT_QQ_FARM_FRIEND_CAROUSEL_RIGHT_SWIPE_TO_COORD = JSON.stringify({ xRatio: 0.74, yRatio: 0.901 });
const DEFAULT_QQ_FARM_FRIEND_CAROUSEL_LEFT_TAP_COORD = JSON.stringify({ xRatio: 0.3, yRatio: 0.885 });
const DEFAULT_QQ_FARM_FRIEND_CAROUSEL_RIGHT_TAP_COORD = JSON.stringify({ xRatio: 0.7, yRatio: 0.885 });
const DEFAULT_QQ_FARM_STORE_ENTRY_COORD = JSON.stringify({ xRatio: 0.10533, yRatio: 0.963 });
const DEFAULT_QQ_FARM_MALL_ENTRY_COORD = JSON.stringify({ xRatio: 0.966, yRatio: 0.151 });
...
const QQ_FARM_FRIEND_CAROUSEL_LEFT_SWIPE_FROM_COORD =
  process.env.WEIXIN_IOS_QQ_FARM_FRIEND_CAROUSEL_LEFT_SWIPE_FROM_COORD?.trim() ||
  DEFAULT_QQ_FARM_FRIEND_CAROUSEL_LEFT_SWIPE_FROM_COORD;
const QQ_FARM_FRIEND_CAROUSEL_LEFT_SWIPE_TO_COORD =
  process.env.WEIXIN_IOS_QQ_FARM_FRIEND_CAROUSEL_LEFT_SWIPE_TO_COORD?.trim() ||
  DEFAULT_QQ_FARM_FRIEND_CAROUSEL_LEFT_SWIPE_TO_COORD;
const QQ_FARM_FRIEND_CAROUSEL_RIGHT_SWIPE_FROM_COORD =
  process.env.WEIXIN_IOS_QQ_FARM_FRIEND_CAROUSEL_RIGHT_SWIPE_FROM_COORD?.trim() ||
  DEFAULT_QQ_FARM_FRIEND_CAROUSEL_RIGHT_SWIPE_FROM_COORD;
const QQ_FARM_FRIEND_CAROUSEL_RIGHT_SWIPE_TO_COORD =
  process.env.WEIXIN_IOS_QQ_FARM_FRIEND_CAROUSEL_RIGHT_SWIPE_TO_COORD?.trim() ||
  DEFAULT_QQ_FARM_FRIEND_CAROUSEL_RIGHT_SWIPE_TO_COORD;
const QQ_FARM_FRIEND_CAROUSEL_LEFT_TAP_COORD =
  process.env.WEIXIN_IOS_QQ_FARM_FRIEND_CAROUSEL_LEFT_TAP_COORD?.trim() ||
  DEFAULT_QQ_FARM_FRIEND_CAROUSEL_LEFT_TAP_COORD;
const QQ_FARM_FRIEND_CAROUSEL_RIGHT_TAP_COORD =
  process.env.WEIXIN_IOS_QQ_FARM_FRIEND_CAROUSEL_RIGHT_TAP_COORD?.trim() ||
  DEFAULT_QQ_FARM_FRIEND_CAROUSEL_RIGHT_TAP_COORD;
const QQ_FARM_STORE_ENTRY_COORD =
  process.env.WEIXIN_IOS_QQ_FARM_STORE_ENTRY_COORD?.trim() || DEFAULT_QQ_FARM_STORE_ENTRY_COORD;
const QQ_FARM_MALL_ENTRY_COORD =
  process.env.WEIXIN_IOS_QQ_FARM_MALL_ENTRY_COORD?.trim() || DEFAULT_QQ_FARM_MALL_ENTRY_COORD;
...
const QQ_FARM_FRIEND_CAROUSEL_DELAY_MS = Number.parseInt(
  process.env.WEIXIN_IOS_QQ_FARM_FRIEND_CAROUSEL_DELAY_MS ?? "1400",
  10,
);
...
const QQ_FARM_FRIEND_CAROUSEL_DIRECTIONS = (process.env.WEIXIN_IOS_QQ_FARM_FRIEND_CAROUSEL_DIRECTIONS ?? "left,right")
  .split(",")
...
function findBestOcrBlock(blocks, candidates, opts = {}) {
  ...
  const matchesCandidate = (normalizedBlock, candidate) =>
  ...
  for (const block of blocks) {
    ...
    for (let index = 0; index < normalizedCandidates.length; index += 1) {
      ...
      const fuzzyMatch = matchesCandidate(normalizedBlock, candidate);
...
function findMatchingOcrBlocks(blocks, candidates, opts = {}) {
  const normalizedCandidates = candidates
    .map((candidate) => normalizeOcrText(candidate))
    .filter(Boolean);
  ...
  const matchesCandidate = (normalizedBlock) =>
...
function pickClosestFriendRowTarget(candidates, anchor, maxRowGap, requireRightSide = false) {
  return [...candidates]
...
async function tryTapQqFarmVisionSidecar(browser, candidates, description, opts = {}) {
  const png = await capturePngScreenshot(browser);
  if (!png) {
    return false;
  }

  const match = await locateQqFarmUiPointWithSidecars(png, candidates, {
    ...opts,
    log,
  });
  if (!match) {
  ...
  const logicalSize = await browser.getWindowSize();
  const point = {
    x: Math.round((match.x / png.width) * logicalSize.width),
    y: Math.round((match.y / png.height) * logicalSize.height),
  };
  await mobileTapPoint(browser, point, description, match.provider);
  return true;
}

async function tryTapQqFarmTemplate(browser, templateNames, description, opts = {}) {
  const png = await capturePngScreenshot(browser);
  if (!png) {
    return false;
  }

  const match = detectBestQqFarmTemplateMatch(png, templateNames, opts);
  if (!match) {
    return false;
  }
  ...
  const logicalSize = await browser.getWindowSize();
  const point = {
    x: Math.round((match.centerX / png.width) * logicalSize.width),
    y: Math.round((match.centerY / png.height) * logicalSize.height),
  };
  await mobileTapPoint(browser, point, description, `模板 ${match.templateName} score=${match.score.toFixed(3)}`);
  return true;
}
...
function detectQqFarmFriendListOverlayFromBlocks(blocks, png) {
...
async function recognizeQqFarmScene(browser) {
  const ocr = await recognizeTextBlocks(browser);
  const png = await capturePngScreenshot(browser);
  const friendListOverlay = ocr && png ? detectQqFarmFriendListOverlayFromBlocks(ocr.blocks, png) : undefined;
  ...
  const visualSnapshot = png ? detectQqFarmVisualSnapshot(png) : { scene: "unknown", markers: {} };
  const ocrDetection = ocr
    ? detectQqFarmSceneFromBlocks(ocr.blocks)
    : {
        scene: "unknown",
        matchedTexts: [],
        matchedGroups: 0,
  ...
  if (visualSnapshot.scene === "friend_farm") {
    return {
      ...
      matchedGroups: ocrDetection.matchedGroups,
    };
  }
  ...
  if (ocrDetection.scene !== "unknown") {
    return ocrDetection;
  }

  if (visualSnapshot.scene === "farm_overview") {
    return {
      scene: "home",
      matchedTexts: Object.keys(visualSnapshot.markers).map((key) => `template:${key}`),
      matchedGroups: 0,
  ...
  if (visualSnapshot.scene === "shop_page" || visualSnapshot.scene === "warehouse" || visualSnapshot.scene === "buy_confirm") {
    return {
      scene: "store",
      matchedTexts: Object.keys(visualSnapshot.markers).map((key) => `template:${key}`),
      matchedGroups: 0,
  ...
  if (png) {
    const sidecarScene = await classifyQqFarmSceneWithSidecars(png, log);
    if (sidecarScene) {
      return {
...
async function clearAppInBackground(browser, bundleId, appLabel, sourceLabel) {
  const terminated = await browser
    .execute("mobile: terminateApp", { bundleId })
    .catch(() => null);

  if (terminated !== true) {
    await browser.closeApp({ bundleId }).catch(() => {});
  }

  await browser.activateApp("com.apple.springboard").catch(() => {});
  log(`已在 iPhone 后台关闭${appLabel}${sourceLabel ? `（${sourceLabel}）` : ""}。`);
}

async function clearDingTalkInBackground(browser, sourceLabel) {
  await clearAppInBackground(browser, DINGTALK_BUNDLE_ID, "钉钉", sourceLabel);
}

async function clearWeChatInBackground(browser, sourceLabel) {
  await clearAppInBackground(browser, WECHAT_BUNDLE_ID, "微信", sourceLabel);
}
...
async function ensureQqFarmMiniProgramOpened(browser) {
  ...
  const opened =
    ...
    await tryTapQqFarmVisionSidecar(browser, QQ_FARM_OPEN_TEXTS, "QQ经典农场前往按钮", {
      minXRatio: 0.55,
      maxXRatio: 0.95,
      minYRatio: 0.15,
      maxYRatio: 0.85,
...
async function resolveFriendVisitTarget(browser) {
  const png = await capturePngScreenshot(browser);
  const ocr = await recognizeTextBlocks(browser);
  const logicalSize = png ? await browser.getWindowSize() : undefined;
  const visitTemplateTargets = png && logicalSize
    ? detectQqFarmTemplateMatches(png, QQ_FARM_FRIEND_VISIT_TEMPLATE_CANDIDATES, {
        minXRatio: 0.52,
        maxXRatio: 0.95,
        minYRatio: 0.16,
        maxYRatio: 0.9,
  ...
  if (ocr?.blocks?.length) {
    const toLogicalPoint = (block) => ({
      x: Math.round((block.centerX / ocr.imageWidth) * ocr.logicalWidth),
      y: Math.round((block.centerY / ocr.imageHeight) * ocr.logicalHeight),
    });
    const visitTextTargets = findMatchingOcrBlocks(ocr.blocks, QQ_FARM_FRIEND_VISIT_TEXTS, {
      minX: ocr.imageWidth * 0.56,
      minY: ocr.imageHeight * 0.18,
      maxY: ocr.imageHeight * 0.9,
    }).map((block) => ({
      ...toLogicalPoint(block),
      text: block.text,
    }));
    const helpTargets = findMatchingOcrBlocks(ocr.blocks, QQ_FARM_FRIEND_HELP_TEXTS, {
      minX: ocr.imageWidth * 0.08,
      maxX: ocr.imageWidth * 0.74,
      minY: ocr.imageHeight * 0.16,
      maxY: ocr.imageHeight * 0.9,
    }).map((block) => ({
      ...toLogicalPoint(block),
      text: block.text,
    }));
    const maxRowGap = ocr.logicalHeight * 0.08;

    for (const helpTarget of helpTargets) {
      const sidecarVisitTarget = png
        ? await locateQqFarmUiPointWithSidecars(png, QQ_FARM_FRIEND_VISIT_TEXTS, {
            minXRatio: 0.52,
            maxXRatio: 0.95,
            minYRatio: Math.max(0, (helpTarget.y - maxRowGap) / ocr.logicalHeight),
            maxYRatio: Math.min(1, (helpTarget.y + maxRowGap) / ocr.logicalHeight),
            log,
          })
        : undefined;
      if (sidecarVisitTarget) {
        return {
          point: {
            x: Math.round((sidecarVisitTarget.x / png.width) * logicalSize.width),
            y: Math.round((sidecarVisitTarget.y / png.height) * logicalSize.height),
          },
          note: `已优先通过${sidecarVisitTarget.provider}拜访需要帮忙好友：${helpTarget.text}`,
        };
      }

      const visitTemplateTarget = pickClosestFriendRowTarget(visitTemplateTargets, helpTarget, maxRowGap, true);
      if (visitTemplateTarget) {
        return {
          point: { x: visitTemplateTarget.x, y: visitTemplateTarget.y },
          note: `已优先通过模板拜访需要帮忙好友：${helpTarget.text}`,
        };
      }

      const visitTextTarget = pickClosestFriendRowTarget(visitTextTargets, helpTarget, maxRowGap, true);
      if (visitTextTarget) {
        return {
          point: { x: visitTextTarget.x, y: visitTextTarget.y },
          note: `已优先拜访需要帮忙好友：${helpTarget.text}`,
        };
      }

      const visitButton = pickClosestFriendRowTarget(candidates, helpTarget, maxRowGap, true);
      if (visitButton) {
        return {
      ...
      return {
        point: {
          x: Math.round(ocr.logicalWidth * 0.36),
          y: helpTarget.y,
        },
        note: `已通过好友条目进入需要帮忙好友农场：${helpTarget.text}`,
      };
    }

    const generalSidecarVisitTarget = png
      ? await locateQqFarmUiPointWithSidecars(png, QQ_FARM_FRIEND_VISIT_TEXTS, {
          minXRatio: 0.52,
          maxXRatio: 0.95,
          minYRatio: 0.16,
      ...
      : undefined;
    if (generalSidecarVisitTarget) {
      return {
        point: {
          x: Math.round((generalSidecarVisitTarget.x / png.width) * logicalSize.width),
          y: Math.round((generalSidecarVisitTarget.y / png.height) * logicalSize.height),
        },
        note: `已通过${generalSidecarVisitTarget.provider}识别好友拜访入口`,
      };
    }

    if (visitTemplateTargets.length > 0) {
      return {
        point: { x: visitTemplateTargets[0].x, y: visitTemplateTargets[0].y },
        note: "已通过模板识别好友拜访入口",
      };
    }
  ...
  const generalSidecarVisitTarget = png && logicalSize
    ? await locateQqFarmUiPointWithSidecars(png, QQ_FARM_FRIEND_VISIT_TEXTS, {
        minXRatio: 0.52,
        maxXRatio: 0.95,
        minYRatio: 0.16,
  ...
  const fallbackPoint = await resolvePoint(browser, QQ_FARM_FRIEND_VISIT_COORD);
  if (!fallbackPoint) {
    return undefined;
  }
  return {
...
async function tapFriendVisitButton(browser) {
  const target = await resolveFriendVisitTarget(browser);
  if (!target) {
    return { visited: false, note: "未识别到好友拜访按钮" };
  }

  await mobileTapPoint(browser, target.point, "QQ农场好友页拜访按钮", "好友模块");
  return { visited: true, note: target.note };
}

async function visitFriendFromBottomCarousel(browser, direction) {
  const rawTap =
    direction === "left" ? QQ_FARM_FRIEND_CAROUSEL_LEFT_TAP_COORD : QQ_FARM_FRIEND_CAROUSEL_RIGHT_TAP_COORD;
  const target = await resolvePoint(browser, rawTap);
  if (!target) {
    return false;
  }

  await mobileTapPoint(browser, target, "QQ农场底部好友栏入口", "好友模块");
  await sleep(QQ_FARM_FRIEND_CAROUSEL_DELAY_MS);
  const detection = await waitForQqFarmScene(browser, 5, 800);
...
async function tryConfirmQqFarmPurchase(browser, description) {
  const confirmedByOcr = await tryTapTextByOcr(browser, QQ_FARM_PURCHASE_CONFIRM_TEXTS, description, {
    minYRatio: 0.5,
    maxYRatio: 0.96,
  });
  if (confirmedByOcr) {
  ...
  return await tryTapQqFarmTemplate(browser, ["btn_buy_confirm"], description, {
    minXRatio: 0.26,
    maxXRatio: 0.82,
    minYRatio: 0.5,
    maxYRatio: 0.96,
...
async function openStore(browser) {
  const openedStore =
    (await tryTapTextByOcr(browser, QQ_FARM_STORE_ENTRY_TEXTS, "QQ农场商店入口", {
      maxXRatio: 0.22,
      minYRatio: 0.85,
    })) ||
    (await mobileTapPointValue(
  ...
  return await waitForStoreScene(browser);
}

async function openWarehouse(browser) {
  const opened =
    (await tryTapTextByOcr(browser, QQ_FARM_WAREHOUSE_ENTRY_TEXTS, "QQ农场仓库入口", {
      maxXRatio: 0.26,
      minYRatio: 0.74,
    })) ||
    (await tryTapQqFarmTemplate(browser, ["btn_warehouse"], "QQ农场仓库入口", {
      maxXRatio: 0.28,
      minYRatio: 0.7,
      minScore: 0.7,
    }));
...
async function openMall(browser) {
  const opened =
    (await tryTapTextByOcr(browser, QQ_FARM_MALL_ENTRY_TEXTS, "QQ农场商城入口", {
      minXRatio: 0.9,
      maxXRatio: 0.995,
      minYRatio: 0.08,
      maxYRatio: 0.28,
    ...
    (await mobileTapPointValue(
      browser,
      QQ_FARM_MALL_ENTRY_COORD,
      "QQ农场商城入口",
      process.env.WEIXIN_IOS_QQ_FARM_MALL_ENTRY_COORD?.trim()
        ? "WEIXIN_IOS_QQ_FARM_MALL_ENTRY_COORD"
        : "默认 QQ 农场商城坐标",
    ));
  if (!opened) {
    return false;
...
async function tryBuySeedFromShopTemplates(browser, notes) {
  ...
  const match = detectBestQqFarmTemplateMatch(png, QQ_FARM_SHOP_TEMPLATE_CANDIDATES, {
    minXRatio: 0.18,
    maxXRatio: 0.82,
    minYRatio: 0.16,
  ...
  await mobileTapPoint(browser, point, "模板商店种子", `模板 ${match.templateName} score=${match.score.toFixed(3)}`);
  await sleep(700);
  ...
  const confirmed = await tryConfirmQqFarmPurchase(browser, "QQ农场模板购种确认按钮");
  if (!confirmed) {
    return undefined;
  }
...
async function buyLatestUnlockedSeed(browser, notes) {
  ...
  if (!latestSeed || !latestSeedPoint) {
    const fallbackPurchased = await tryBuySeedFromShopTemplates(browser, notes);
    if (fallbackPurchased) {
      const sceneAfterFallback = await recognizeQqFarmScene(browser);
  ...
  const confirmed = await tryConfirmQqFarmPurchase(browser, "QQ农场购种确认按钮");
  if (!confirmed) {
    await closeStoreModal(browser);
    return undefined;
  }
...
async function runWarehouseModule(browser) {
  const notes = [];
  if (!(await openWarehouse(browser))) {
    notes.push("未识别到仓库入口");
    return { notes };
  }
  ...
  notes.push("已打开仓库");
  const sold =
    (await tryTapTextByOcr(browser, QQ_FARM_WAREHOUSE_SELL_TEXTS, "QQ农场仓库一键出售", {
      minYRatio: 0.46,
      maxYRatio: 0.96,
    })) ||
    (await tryTapQqFarmTemplate(browser, ["btn_batch_sell", "btn_sell"], "QQ农场仓库一键出售", {
      minXRatio: 0.18,
      maxXRatio: 0.88,
      minYRatio: 0.4,
      maxYRatio: 0.96,
  ...
  await sleep(900);
  const confirmed = await tryConfirmQqFarmPurchase(browser, "QQ农场仓库一键出售确认按钮");
  if (confirmed) {
    await sleep(900);
...
async function runMallModule(browser) {
  const notes = [];
  if (!(await openMall(browser))) {
    notes.push("未识别到商城入口");
    return { notes };
  }

  notes.push("已打开商城");
  const openedFertilizer =
    (await tryTapTextByOcr(browser, QQ_FARM_MALL_FERTILIZER_TEXTS, "QQ农场10小时有机化肥", {
      minXRatio: 0.12,
      maxXRatio: 0.9,
      minYRatio: 0.18,
      maxYRatio: 0.94,
    })) ||
    (await tryTapQqFarmTemplate(browser, ["bth_feiliao2_yj", "btn_fertilize_popup"], "QQ农场10小时有机化肥", {
      minXRatio: 0.08,
      maxXRatio: 0.92,
      minYRatio: 0.14,
      maxYRatio: 0.96,
  ...
  await sleep(900);
  const confirmed = await tryConfirmQqFarmPurchase(browser, "QQ农场购买10小时有机化肥确认按钮");
  if (confirmed) {
    await sleep(900);
...
async function runStoreModule(browser) {
  const notes = [];
...
async function runFriendAccessModule(browser) {
  const notes = [];
  ...
  if (friendScene.scene !== "friends") {
    notes.push(`未确认进入好友页（当前场景 ${friendScene.scene}）`);
    return { notes };
  }
  notes.push("已打开好友列表");

  const visited = await tapFriendVisitButton(browser);
  notes.push(visited.note);
  if (!visited.visited) {
    await mobileTapPointValue(
      browser,
      QQ_FARM_FRIEND_POPUP_CLOSE_COORD,
    ...
    await sleep(600);
    const retriedVisited = await tapFriendVisitButton(browser);
    notes.push(`好友拜访重试：${retriedVisited.note}`);
    if (!retriedVisited.visited) {
      return { notes };
    }
  }

  await sleep(QQ_FARM_POST_VISIT_DELAY_MS);
  let visitedScene = await waitForQqFarmScene(browser, 8, 1_000);
  if (visitedScene.scene !== "friend-farm") {
    notes.push(`未确认进入好友农场（当前场景 ${visitedScene.scene}）`);
    return { notes };
  }
  notes.push("已拜访好友农场");
  ...
  for (const direction of QQ_FARM_FRIEND_CAROUSEL_DIRECTIONS) {
    const switched = await visitFriendFromBottomCarousel(browser, direction);
    if (!switched) {
    ...
    visitedScene = await waitForQqFarmScene(browser, 3, 600);
    if (visitedScene.scene !== "friend-farm") {
      break;
    }
  }
...
async function runFriendModules(browser) {
  const modules = [
    {
      id: "friend-access",
      name: "访问好友操作模块",
      run: runFriendAccessModule,
    },
  ];
...
async function runDefaultQqFarmRoutine(browser) {
  ...
  notes.push(...await runFriendModules(browser));
  return {
    friendVisited: notes.some((note) => note === "已拜访好友农场"),
    message: notes.join("，"),
  };
}
...
async function runQqFarm(browser) {
  ...
  if (canvasSteps.length > 0) {
    ...
    let message = "已打开 QQ经典农场，并执行配置的画布点击步骤。";
    if (CLEAR_WECHAT_AFTER_QQ_FARM) {
      await clearWeChatInBackground(browser, "QQ农场后");
      message = `${message.slice(0, -1)}，已在 iPhone 后台关闭微信。`;
    }
    console.log(message);
    return;
  }
  ...
  let message = `已打开 QQ经典农场 小程序，${routineResult.message}`;
  if (CLEAR_WECHAT_AFTER_QQ_FARM) {
    await clearWeChatInBackground(browser, "QQ农场后");
    message = `${message}，已在 iPhone 后台关闭微信`;
  }
  console.log(message);
}
