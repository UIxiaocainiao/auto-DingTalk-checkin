...
function parseSeedQuality(text) {
...
function parseUnlockLevel(text) {
...
function compareBlocksTopLeft(left, right) {
  return left.centerY - right.centerY || left.centerX - right.centerX;
}

function findSeedCardBlocks(blocks, qualityBlock) {
...
function findSeedLabelBlock(blocks, qualityBlock) {
  return blocks
    .filter(
      (block) =>
...
function findSeedDetailBlock(blocks, qualityBlock) {
  return blocks
    .filter(
      (block) =>
...
function resolveSeedTapPoint(qualityBlock, labelBlock, detailBlock) {
...
export function parseQqFarmStoreSeeds(blocks, options = {}) {
...
export function pickLatestUnlockedQqFarmStoreSeed(seeds) {
  return [...seeds]
...
export function describeQqFarmStoreSeeds(seeds) {
  if (seeds.length === 0) {
    return "未识别到商店种子";
  }
